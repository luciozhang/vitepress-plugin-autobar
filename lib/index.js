(function (factory) {
    ...
    function side(baseDir, options) {
        ...
        var baseURL = determineBaseURL(baseDir);
        var sidebars = [];
        ...
        mdFiles.forEach(function (item) {
            ...
            var link = baseURL ? "".concat(baseURL, "/").concat(item) : item;
    ...
    function determineBaseURL(baseDir) {
        var normalizedBaseDir = baseDir.replace(/^\.\//, '');
        var parts = normalizedBaseDir.split('/');
        if (parts.length > 1) {
